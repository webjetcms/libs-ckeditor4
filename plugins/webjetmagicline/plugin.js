...
( function() {
	CKEDITOR.plugins.add( 'webjetmagicline', {
	...
	function initLastSpanPlaceholder( editor ) {
	...
	function initPlugin( editor ) {
		...
		try {
			initLastSpanPlaceholder( editor );
		} catch ( e ) {
			console.error( 'Error initializing webjetmagicline.initLastSpanPlaceholder', e );
		}
		...
		function addListeners() {
			...
			editable.attachListener( editor, 'beforeUndoImage', function(event) {
				that.line.detach();
				that.line.lineChildren[ 1 ].hide();
			} );
	...
	function initLine( that ) {
		...
		extend( line, {
			...
			lineChildren: [
				extend(
					newElementFromHtml(
						...
						base: CSS_COMMON + 'height:17px;width:17px; left: 50%; right 50%; transform: translateX(-50%);' +
								...
								'font-size: 16px;line-height:17px;border:1px solid ' + that.boxColor + ';text-align:center;',
						looks: [
				...
				extend(
						newElementFromHtml(
							...
							base: CSS_COMMON + 'height:56px;width:224px; left: 50%; right 50%; transform: translateX(-50%);box-sizing:content-box;' +
								...
								'font-size: 16px;line-height:17px;border:1px solid ' + that.boxColor + ';text-align:center; display: none;',
							looks: [
								'top:12px; border-radius: 2px;',
								'top:-78px; border-radius: 2px 2px 0px 0px;',
								'top:21px; border-radius: 0px 0px 2px 2px;'
							]
						}
				),
				extend( newElementFromHtml( TRIANGLE_HTML, doc ), {
					...
					looks: [
						'border-width:4px 0 4px 4px;top:-4px',
						'border-width:4px 0 0 4px;top:-4px',
						'border-width:0 0 4px 4px;top:0px'
					]
				} ),
				extend( newElementFromHtml( TRIANGLE_HTML, doc ), {
					base: CSS_TRIANGLE + 'right:0px;border-right-color:' + that.boxColor + ';',
					looks: [
						'border-width:4px 4px 4px 0;top:-4px',
						'border-width:4px 4px 0 0;top:-4px',
						'border-width:0 4px 4px 0;top:0px'
					]
				} )
			],
			...
			place: function() {
				...
				// Set Y coordinate (top) for single-edge trigger.
					...
					styleSet.top = lower.size.top; //webjet - robilo problem vo vnorenych divoch s marginom 30 - lower.size.margin.top;
				else if ( !lower ) {
					styleSet.top = upper.size.bottom; //webjet - robilo problem vo vnorenych divoch s marginom 30 + upper.size.margin.bottom;
				}
		...
		//console.log(line);
		//console.log(line.lineChildren);

		line.lineChildren[ 0 ].on( 'mouseup', function( event )
		{
			line.lineChildren[ 1 ].forEach( function(node)
			{
				//console.log(node);

				if (node.$.tagName == "LI")
				{
					//console.log(node.getAttribute("data-wjmouseup"));

					if (node.getAttribute("data-wjmouseup") == null)
					{
						node.setAttribute("data-wjmouseup", "1");

						node.on( 'mouseup', function(event)
						{

							var command = node.getAttribute("data-wjcommand");
