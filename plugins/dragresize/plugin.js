...
(function() {
  ...
  CKEDITOR.plugins.add(PLUGIN_NAME, {
    onLoad: function() {
      ...
      if (!isWebkit || navigator.userAgent.indexOf("Edge")!=-1) {
    	  console.log("dragresize not loaded 1");
        return;
      }
    ...
    init: function(editor) {
      if (!isWebkit || navigator.userAgent.indexOf("Edge")!=-1) {
    	  console.log("dragresize not loaded 2");
        return;
      }
      //onDomReady handler
  ...
  Resizer.prototype = {
    ...
    show: function(el) {
      ...
      if (this.document.body==null) {
        //console.log("el=", el, "this.document.body=", this.document.body, "container=", this.container);
        return;
      }
  ...
  {
	  //WebJET updrava pre thumb obrazky
    ...
    if (el.tagName && el.tagName.toLowerCase()=="img")
    {
    	...
    	if (src && src.indexOf("/thumb")==0)
    	{
    		var i = src.indexOf("?");
    		if (i > 0) src = src.substring(0, i);
    		src += "?v="+Date.now();
    		src += "&w="+width+"&h="+height+"&ip=6";
